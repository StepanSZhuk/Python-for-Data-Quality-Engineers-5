...
class UploadFolder:
    def upload_folder(self):
        print(f'Define file path for downloading file(s): \nAvaliable list: \n1.Default folder '
...
class UploadToDatabase:
    def upload_to_database(self):
        ...
        cursor.execute("create table if not exists news (id INTEGER PRIMARY KEY AUTOINCREMENT, text_news text, city text, date text)")
        cursor.execute('create table if not exists private_ad (id INTEGER PRIMARY KEY AUTOINCREMENT,text_private_ad text, expire_date text)')
        cursor.execute('create table if not exists tip_of_the_day (id INTEGER PRIMARY KEY AUTOINCREMENT, text_tip text)')
            ...
            if file.endswith(".xml"):
                ...
                for publication in root:
                    if publication.attrib['name'].lower() == 'news':
                        ...
                        cursor.execute("select * from news where text_news = ? and  city = ? and date = ?", [text, city, date])
                        duplicates = cursor.fetchall()
                        if len(duplicates) == 0:
                            cursor.execute("insert into news (text_news, city, date) values (?, ?, ?)", [text, city, date])
                            connection.commit()
                            print(f'\nThis news is added to database\n')
                        else:
                            print(f'This record already exists in the database!')
                    elif publication.attrib['name'].lower() == 'private ad':
                        text = publication.find('text').text
                        expire_date = publication.find('expire_date').text
                        cursor.execute("select * from private_ad where text_private_ad = ? and  expire_date = ?", [text, expire_date])
                        duplicates = cursor.fetchall()
                        if len(duplicates) == 0:
                            cursor.execute("insert into private_ad (text_private_ad, expire_date) values (?, ?)", [text, expire_date])
                            connection.commit()
                            print(f'\nThis ad  is added to database\n')
                        else:
                            print(f'This record already exists in the database!')
                    elif publication.attrib['name'].lower() == 'tip of the day':
                        text = publication.find('text').text
                        cursor.execute("select * from tip_of_the_day where text_tip = ?", [text])
                        duplicates = cursor.fetchall()
                        if len(duplicates) == 0:
                            cursor.execute("insert into tip_of_the_day (text_tip) values (?)", [text])
                            connection.commit()
                            print(f'\nThis tip is added to database\n')
                        else:
                            print(f'This record already exists in the database!')
                if os.path.exists(file_path):
                    os.remove(file_path)
                ...
                with open(file_path, 'r') as json_file:
                    ...
                    for publication in json_data:
                        if publication == 'news':
                            text = json_data[publication]['text']
                            city = json_data[publication]['city']
                            date = json_data[publication]['date']
                            cursor.execute("select * from news where text_news = ? and  city = ? and date = ?",
                                               [text, city, date])
                            duplicates = cursor.fetchall()
                            if len(duplicates) == 0:
                                cursor.execute("insert into news (text_news, city, date) values (?, ?, ?)",
                                                   [text, city, date])
                                connection.commit()
                                print(f'\nThis news is added to database\n')
                            else:
                            ...
                            text = json_data[publication]['text']
                            expire_date = json_data[publication]['expire_date']
                            cursor.execute("select * from private_ad where text_private_ad = ? and expire_date = ?",[text, expire_date])
                            duplicates = cursor.fetchall()
                            if len(duplicates) == 0:
                                cursor.execute("insert into private_ad (text_private_ad, expire_date) values (?, ?)",[text, expire_date])
                            ...
                            if len(duplicates) == 0:
                                ...
                                print(f'This record already exists in the database!')
                            # cursor.execute('select * from tip_of_the_day')
                            # res = cursor.fetchall()
                            # print(res)
                            print(f'\nThis tip is published\n')
                if os.path.exists(file_path):
...
class ChooseElement(News, PrivateAd, PublishTips, UploadFile, CsvParsing, JsonModule, Xml, UploadToDatabase):
    @staticmethod
    def choose_ele():
        while True:
            try:
